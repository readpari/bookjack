    ...
    <div
      ...
      onClick={() => {
        if (isElectron && StorageUtil.getReaderConfig('isOpenInMain') !== 'yes') {
          if (window.require('electron').ipcRenderer.sendSync('check-main-open', 'ping')) {
            window.require('electron').ipcRenderer.invoke('focus-on-main', 'ping');
            window.close();
          } else {
            window.require('electron').ipcRenderer.invoke('create-new-main', 'ping');
            window.close();
          }
        } else {
