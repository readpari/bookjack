      ...
      style={{
        backgroundColor:
          props.format === 'PDF'
            ...
            ? ' #6867d1'
            : props.format === 'CBT' || props.format === 'CBZ' || props.format === 'CBR'
            ? '#00b6c2'
            : 'rgba(104, 103, 209, 1)',
      }}>
      {props.format || 'BOOK'}
    </div>
    <div className="cover-title" style={{ fontSize: props.scale === 1.15 ? '13px' : '' }}>
      {props.title}
    </div>
    <div className="cover-footer">Book Jack</div>
  </div>
);
